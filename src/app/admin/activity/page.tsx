...
import {
  SortableHeader,
  TableHeader,
} from "@/app/components/ui/SortableHeader";
...
export default function ActivityPage() {
  ...
  const [sortConfig, setSortConfig] = useState<{
    key: keyof ActivityListItem;
    direction: "asc" | "desc" | null;
  }>({ key: "name_th", direction: "asc" });
  ...
  const currentItems = processedActivities.slice(
  ...
  const favoriteCount = useMemo(
    () => activities.filter((a) => a.favorite).length,
    [activities],
  );
          ...
          <table className="w-full text-left border-collapse table-fixed min-w-[950px]">
            <thead className="z-20 bg-slate-50 dark:bg-slate-900 border-b border-slate-200 dark:border-slate-800 text-[11px] uppercase tracking-wider text-slate-500 font-bold">
              <tr>
                ...
                />
                <th className="px-6 py-3 w-28 text-right pr-8 sticky right-0 z-20 bg-slate-50 dark:bg-slate-900 border-l border-slate-200 shadow-[-4px_0_10px_rgba(0,0,0,0.02)]">
                  จัดการ
                </th>
              </tr>
            </thead>
              ...
              {loading ? (
                ...
                currentItems.map((activity, index) => (
                  ...
                  >
                    ...
                    <td
                      className={`px-6 py-4 text-right pr-8 sticky right-0 z-10 border-l border-slate-100 transition-colors shadow-[-4px_0_10px_rgba(0,0,0,0.02)] ${index % 2 === 0 ? "bg-white dark:bg-slate-950" : "bg-slate-50 dark:bg-slate-900"} group-hover:bg-sky-50 dark:group-hover:bg-slate-800`}
                    >
                      <div className="flex justify-end gap-1.5 relative z-20">
                        <Link
                          href={`/admin/activity/edit/${activity._id}`}
